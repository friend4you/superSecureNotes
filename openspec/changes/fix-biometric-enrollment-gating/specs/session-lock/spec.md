...
On lock, the system SHALL call `notesIndexStore.close()`, then `vaultSession.clear()`, and clear in-memory auth session state (`currentSession`, `currentUser`). The system SHALL clear the session password cache. The system SHALL NOT modify Keychain contents on lock and SHALL NOT clear the `pendingBiometricEnrollment` UserDefaults flag on lock.
...
#### Scenario: Lock clears session password cache

- **WHEN** a lock event occurs while the session password cache contains a password
...
#### Scenario: Lock preserves pending enrollment flag

- **WHEN** a lock event occurs while `pendingBiometricEnrollment` is `true`
- **THEN** `pendingBiometricEnrollment` remains `true`

### Requirement: Logout full reset

Logout SHALL call `notesIndexStore.close()`, `credentialStore.clearAll()`, `vaultSession.clear()`, clear in-memory auth state, clear the session password cache, clear the pending biometric enrollment flag, and navigate to the login screen with editable email.
...
#### Scenario: Logout clears session password cache

- **WHEN** the user taps logout
- **THEN** the session password cache is cleared

#### Scenario: Logout clears pending enrollment flag
