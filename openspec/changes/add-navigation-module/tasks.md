...
- [x] 14.1.1 Write failing tests: `Navigating` inherits `NavigationRouting`; includes `dismissPresentation()` (`NavigationProtocolTests`)
- [x] 14.1.2 Implement `Navigating` protocol in `NavigationProtocol`

### 14.2 Navigation — AppNavigator

- [x] 14.2.1 Write failing tests: `AppNavigator.push` validates route type is registered before delegating; unregistered type does not mutate router (`NavigationTests`)
- [x] 14.2.2 Write failing tests: `AppNavigator` delegates `setRoot`, `present`, `pop`, `popToRoot`, `dismissPresentation` to internal router (`NavigationTests`)
- [x] 14.2.3 Implement `AppNavigator` conforming to `Navigating`

### 14.3 Navigation — RouteRegistry verifyRegistered

- [x] 14.3.1 Write failing tests: `verifyRegistered` passes when all expected types registered; fails in debug when a type is missing (`NavigationTests`)
- [x] 14.3.2 Implement `RouteRegistry.verifyRegistered(_:)` using auto-tracked registration from `register()`

### 14.4 Navigation — Coordinator and environment cleanup

- [x] 14.4.1 Update `NavigationCoordinator` to expose `navigator: Navigating`; make `router` internal
- [x] 14.4.2 Remove `NavigationRouterEnvironment.swift` and environment injection from `NavigationHost`
- [x] 14.4.3 Update `NavigationTests` and `NavigationHost` tests for coordinator API change

### 14.5 AuthFlow — replace LoginNavigating with Navigating on deps

- [x] 14.5.1 Write failing tests: `DefaultLoginViewModel.registerTapped()` calls `navigator.push(AuthRoute.register)` via deps (`AuthFlowProtocolTests`)
- [x] 14.5.2 Add `NavigationProtocol` dependency to `AuthFlowProtocol`; add `navigator: Navigating` to `AuthFlowDependencies` init
- [x] 14.5.3 Remove `LoginNavigating`, `AuthLoginNavigator`; update `AuthFlowDependencyProviding.makeLoginViewModel()` to take no navigator param
- [x] 14.5.4 Update `AuthNavigation.view(for:deps:)` and `RouteRegistry+AuthRoutes` to remove navigator parameter
- [x] 14.5.5 Update `LoginViewRoutingTests`, previews, and mocks to use `MockNavigating`

### 14.6 NotesFlow — Navigating on deps

- [x] 14.6.1 Add `NavigationProtocol` dependency to `NotesFlow`; add `navigator: Navigating` to `NotesFlowDependencies` init
- [x] 14.6.2 Update `NotesFlowTests` and mocks as needed

### 14.7 App composition wiring

- [x] 14.7.1 Refactor `AppComposition` to create coordinator first, pass `coordinator.navigator` into deps inits, register routes without navigator param
- [x] 14.7.2 Call `registry.verifyRegistered(AuthRoute.self, NotesRoute.self)` after registration (debug)
- [x] 14.7.3 Update `SessionRootNavigation` to use `navigator.setRoot(...)` instead of `router`

### 14.8 Final verification

- [x] 14.8.1 Run full test suite; fix regressions from `LoginNavigating` removal and coordinator API change
