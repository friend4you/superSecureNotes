...
final class ByteBufferTests: XCTestCase {
    ...
    func testInsufficientDataOnRead() {
        var reader = ByteBuffer(data: Data([0, 0, 0, 5]))

        XCTAssertThrowsError(try reader.readLengthPrefixedBytes()) { error in
            XCTAssertEqual(error as? SecureCryptoError, .insufficientData)
        }
    }
